...
1. **Develop on a version branch** (e.g. `2.1.0`). Run `pnpm version-bump:patch|minor|major` on it — the existing `scripts/version-bump/index.ts` keeps `package.json`, Android `versionCode`/`versionName`, and iOS `MARKETING_VERSION`/`CURRENT_PROJECT_VERSION` in sync.
2. **Open PR `2.1.0` → `main`.** The existing `tests.yml` workflow (lint, audit, build, unit, e2e) gates the merge. Merge it.
...
- [ ] **[Phase 1 — Release workflow + GitHub Release](phase-1-release-workflow-and-github-release.md)**: the button → tag + GitHub Release with apk/aab/ipa/web artifacts and auto-changelog
...
- Only existing CI: `.github/workflows/tests.yml`. There is **no** release/deploy automation yet.
- Version source of truth: `package.json` (`2.0.0`), synced by `scripts/version-bump/index.ts`. Android is at `versionCode 19 / versionName 2.0.0` (`android/app/build.gradle`), iOS at `MARKETING_VERSION 2.0.0 / CURRENT_PROJECT_VERSION 19` (`ios/App/App.xcodeproj/project.pbxproj`).
...
- `pnpm build:android` / `build:ios` end with `npx cap open …` (interactive) — CI uses `cap sync` + Gradle/xcodebuild directly instead.
