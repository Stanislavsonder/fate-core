...
| 4 | [phase-4-sdk-extensions.md](./phase-4-sdk-extensions.md) | Published author SDK (scaffolder, build preset, types) + custom dice capability. **`@fate-core/mod-types`/`@fate-core/mod-build` are already published to npm** (done as an unplanned Phase 3 prerequisite — CI's smoke-load step and real authors' local dev both needed it). Remaining scope here: the scaffolder CLI and custom-dice capability. |
