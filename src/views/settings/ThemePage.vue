...
<template>
	<ion-page>
		...
		<ion-content>
			<ion-list :inset="true">
				<ion-item v-for="themeOption in THEMES" :key="themeOption.name" button :detail="false" @click="setTheme(themeOption.name)">
					<ion-radio slot="start" :checked="themeOption.name === theme" class="pointer-events-none"/>
					<ion-icon slot="start" :icon="themeOption.icon"/>
